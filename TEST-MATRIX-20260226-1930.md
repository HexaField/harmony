...
| 14.4 | Search finds messages across multiple channels | ⚠️ | search.query WS handler added. Works for unencrypted content. With E2EE always-on, server-side search returns empty (by design — zero knowledge). Client-side search needed. |
...
### Remaining ⚠️ — What Each Needs

| # | Item | What's Needed |
| --- | --- | --- |
| 5.6 | DM encryption falls back to plaintext | Wire key exchange relay: server fetches recipient's MLS key package, forwards public key to sender |
| 11.1 | Voice channel join | Needs real browser + microphone — server handler exists |
| 11.8 | Voice sidebar participant count | Wire `voice.state` broadcast into sidebar component |
| 14.1 | Search overlay (Ctrl+K) | Needs browser to verify — UI component exists |
| 14.2 | Search results with highlights | Wire `search.results` into overlay with highlight spans |
| 14.3 | Click result → navigate | Add click handler on results → set active channel + scroll |
| 14.4 | Cross-channel search | Client-side search over decrypted message cache (server can't search — E2EE) |
