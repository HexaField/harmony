...
| 1.5 | Open in new incognito tab → onboarding shown (no identity) | ⚠️ | Cannot open incognito in automated browser. Verified identity is stored in localStorage; clearing it causes onboarding to reappear, confirming the mechanism works. |
...
| 1.7 | Set display name in settings → name shown in UI | ⚠️ | Set display name "TestUser" in Settings → Identity. Name updated in sidebar ("TE TestUser") but member list still shows DID suffix "cMGTETqu" — partial propagation |
| 1.8 | Pseudonym shown when no display name is set | ❌ | User shows DID suffix "cMGTETqu" instead of a two-word pseudonym. Other members DO get pseudonyms (e.g. "Amber Shark", "Onyx Falcon") but the local user's own pseudonym is not generated/displayed. |
...
| 2.7 | Leave community → removed from server list | ❌ | No "Leave Community" action in UI. The ▾ next to community name is decorative (no dropdown). No right-click context menu on server icons. Feature not implemented. |
...
| 2.10 | Community settings accessible (name/description editable) | ❌ | `CommunitySettingsForm` is a stub (returns plain object, no JSX). No gear icon or menu to access community settings. ▾ dropdown not wired. |
...
| 3.7 | Channel changes broadcast to other connected members | ⚠️ | All communities show "Disconnected" (WebSocket not reconnecting in automated browser). Verified channel changes persist after full page reload — state is server-persisted. Could not test live broadcast to second client due to disconnected state. |
...
| 4.2 | Second user sees message appear in real-time (no refresh) | ⚠️ | Programmatic client sent message — it appeared in browser in real-time, but displayed as `[encrypted]` due to E2EE (separate MLS key groups per client). Sender name "Mint Wolf" and timestamp visible. Real-time delivery works; cross-client decryption does not (by design for separate identities without key exchange) |
...
| 4.6 | Other user sees edit/delete in real-time | ⚠️ | Programmatic client successfully called editMessage() and deleteMessage() APIs (no errors). Browser showed encrypted messages so edit/delete visibility could not be verified visually. Server-side operations succeeded |
| 4.7 | Typing indicator: user A types → user B sees "typing..." | ⚠️ | sendTyping() called from programmatic client without error. No visible typing indicator appeared in browser UI — may require same MLS group or UI component may not render for encrypted channels. API exists but UI display unverified |
| 4.8 | Add reaction to message → reaction count shown on message | ⚠️ | Clicked 😀 → emoji picker appeared with 👍 ❤️ 😂 🎉 😮 😢 🔥 👀. Clicked 👍 — picker closed but no visible reaction count badge appeared on message. Reaction UI component exists in code (MessageArea.tsx shows emoji+count badges) but may not be wired to the store correctly |
| 4.9 | Remove reaction → reaction removed | ⚠️ | Could not verify — reaction badge never appeared visually (see 4.8). removeReaction() API called successfully from programmatic client |
| 4.10 | Other user sees reactions appear/disappear in real-time | ⚠️ | addReaction()/removeReaction() called from programmatic client without error. Browser did not display reaction badges for any messages — cannot confirm cross-user visibility |
| 4.11 | Reply to message → reply shown with reference to parent | ⚠️ | 💬 button opens a **Thread creation** dialog (textbox "Thread name" + "Create Thread"), not an inline reply. MessageContextMenu component has a "Reply" action but is not wired into MessageArea.tsx. No inline reply-to-message UI available |
...
| 4.15 | Message context menu appears on right-click/long-press | ❌ | MessageContextMenu component exists in code (packages/ui/src/components/Channel/MessageContextMenu.tsx) with Reply, React, Thread, Pin, Edit, Delete options — but it is NOT imported or used in MessageArea.tsx. Right-click shows browser default context menu. Actions are available via hover buttons (😀 ✏️ 🗑️ 💬) instead |
...
| 5.5 | DM typing indicator works | ❌ | Server has `dm.typing` handler but client has NO public `sendDMTyping()` method — missing client API. |
| 5.6 | DM messages are encrypted (verify via protocol: server payload contains ciphertext, not plaintext) | ⚠️ | E2EE pathway exists (`encryptForDM` → `SimplifiedDMProvider` → X25519) but falls back to plaintext when recipient public key unavailable. No key package discovery infra — DMs sent as plaintext in practice. |
| 5.7 | DM list shows conversations with unread indicators | ⊘ | Unread count partially implemented |
| 5.8 | DM edit → updated for recipient | ❌ | Client has `editDM()` method but server has NO `dm.edit` handler — silently dropped. |
| 5.9 | DM delete → removed for recipient | ❌ | Client has `deleteDM()` method but server has NO `dm.delete` handler — silently dropped. |
...
| 6.3 | Parent message shows reply count (e.g., "3 replies") | ⚠️ | Server increments thread.messageCount internally but no explicit reply-count broadcast to clients |
...
| 9.2 | Create new role with name and permissions → role appears in list | ❌ | createRole() timed out waiting for role.created (no broadcast received) |
| 9.3 | Assign role to member → member's roles updated | ⚠️ | Client.assignRole sends member.update; server expects role.assign → no member update observed |
...
| 9.5 | Member with correct role → can perform permitted action | ⚠️ | Cannot verify due to role assignment API mismatch |
| 9.6 | Delete role → removed from all members who had it | ⚠️ | No role to delete (9.2 failed) |
...
| # | User Flow | Result | Comments |
| --- | --- | --- | --- |
| 10.1 | Ban member → member disconnected, cannot rejoin | ⚠️ | Client.banMember sends "member.ban" but server handles "community.ban" — message type mismatch |
| 10.2 | Banned member sees appropriate error when attempting to reconnect | ⚠️ | Skipped — depends on 10.1 (ban not executed due to API mismatch) |
| 10.3 | Unban member → member can rejoin | ⚠️ | Skipped — depends on 10.1 |
| 10.4 | Kick member → member removed but can rejoin | ⚠️ | No server handler for "member.kick" (client.kickMember message dropped) |
| 10.5 | Banned user's messages rejected by server | ⚠️ | Server has ban check in channel.send but cannot test end-to-end due to ban API mismatch |
| 10.6 | Rate limiting: rapid message sends → rate limit error shown | ⊘ | Rate limiting not configured on test instance |
...
| # | User Flow | Result | Comments |
| --- | --- | --- | --- |
| 11.1 | Click voice channel → join, voice control bar appears | ⚠️ | Voice join requires UI + media devices — server handler exists (voice.join) |
| 11.2 | Second user joins same channel → both see each other in participant list | ⚠️ | Participant list requires UI + two media clients |
| 11.3 | Mute button toggles mute → state reflected for other participant | ⚠️ | Mute toggle requires media devices — server handler exists (voice.mute/unmute) |
| 11.4 | Video button toggles camera → video grid shows/hides stream | ⚠️ | Video toggle requires camera — server handler exists (voice.video) |
| 11.5 | Screen share button → screen share stream visible to others | ⚠️ | Screen share requires display media — server handler exists (voice.screen) |
| 11.6 | Speaking indicator lights up when audio detected | ⚠️ | Speaking indicator requires audio analysis — server handler exists (voice.speaking) |
| 11.7 | Leave voice → removed from participant list, control bar hidden | ✅ | Voice leave sent — server handler processes cleanly |
| 11.8 | Voice channel in sidebar shows participant count/avatars | ⚠️ | Sidebar voice indicators require UI rendering |
...
| 11.11 | LiveKit SFU | ⊘ | Requires LiveKit server |
...
| 12.3 | Send message → verify server-side payload is ciphertext (not readable plaintext) | ⚠️ | No channel.history WS handler to verify server-side payload; encryption confirmed in code |
...
| 13.2 | Select file → preview chip shown in composer | ⊘ | Native file picker blocks automation — needs manual file selection |
...
| 13.4 | Other user sees attachment in real-time | ⚠️ | Broadcast verified in code — media.upload.complete sent to community subscribers |
| 13.5 | File > 10MB → error shown, upload rejected | ✅ | File > 10MB rejected with error |
| 13.6 | Multiple attachments on single message | ⚠️ | Multiple attachments supported via multiple media.upload.request calls |
...
| # | User Flow | Result | Comments |
| --- | --- | --- | --- |
| 14.1 | Open search overlay (Ctrl+K or search icon) | ⚠️ | Search overlay is UI-only (Ctrl+K) |
| 14.2 | Type query → results shown with snippet highlights | ⚠️ | Search results rendering is UI-only |
| 14.3 | Click result → navigated to message in channel | ⚠️ | Navigation to search result is UI-only |
| 14.4 | Search finds messages across multiple channels | ⚠️ | No search.query WS handler — search is server-side only via MessageStore.search() |
| 14.5 | Search with no results shows empty state | ⚠️ | Empty state display is UI-only — server search returns empty array for no matches |
...
| # | User Flow | Result | Comments |
| --- | --- | --- | --- |
| 15.1 | Open migration wizard in UI | ⊘ | No migration wizard UI found |
| 15.2 | Upload Discord export file → parsed and previewed | ⊘ | No Discord import WS handler — reconcileMember() exists for ghost member linking |
| 15.3 | Confirm import → community created with channels and message history | ⊘ | No migration.import handler on server |
| 15.4 | Imported messages visible in channels | ⊘ | Migration not implemented as WS flow |
| 15.5 | Re-import same data → dedup prevents duplicates | ⊘ | Dedup not testable — no import handler |
| 15.6 | Discord bot migration | ⊘ | Requires Discord bot token |
| 15.7 | GDPR / privacy features | ⊘ | Not implemented |
...
| 16.5 | Non-member attempts action → rejected by server | ⚠️ | Non-member send silently dropped — server checks membership in handleChannelSend |
...
| 17.3 | Leave community → VC revoked (verify via revocation store) | ⚠️ | Leave processed — no explicit revocation store, server removes membership record |
...
| 18.5 | Client reconnects after server restart (exponential backoff) | ⊘ | Requires server restart during test |
| 18.6 | Config file loads correctly | ✅ | Server started and running on :9999 with default config |
| 18.7 | SQLite store persists data across server restart | ⊘ | Requires server restart during test |
| 18.8 | Health check endpoint responds | ❌ | HTTP /health and /healthz return 426 (Upgrade Required) on :9999 — no HTTP health endpoint |
