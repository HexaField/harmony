...
| 4.7 | Typing indicator: user A types → user B sees "typing..." | ✅ | Programmatic: typing broadcast received by second client |
| 4.8 | Add reaction to message → reaction count shown on message | ⚠️ | Server broadcasts channel.reaction.added correctly. Client emits as generic "message" event. UI reaction badges not rendering — store wiring bug. |
| 4.9 | Remove reaction → reaction removed | ⚠️ | Server has channel.reaction.remove handler. UI doesn't render badges so removal not visible. |
| 4.10 | Other user sees reactions appear/disappear in real-time | ⚠️ | Server broadcasts reactions correctly. UI store wiring issue prevents display. |
...
| 11.2 | Second user joins same channel → both see each other in participant list | ⊘ | Requires two media clients with audio devices |
| 11.3 | Mute button toggles mute → state reflected for other participant | ⊘ | Requires media devices |
